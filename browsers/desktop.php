...
function desktop_theme_status_form($text = '', $in_reply_to_id = NULL, $is_desktop = true) {
	if (user_is_authenticated()) {
		...
		$output = '<form method="post" action="'.BASE_URL.'update"><textarea id="status" name="status" rows="3" style="width:100%; max-width: 400px;">'.$text.$fixedtags.'</textarea><div><input name="in_reply_to_id" value="'.$in_reply_to_id.'" type="hidden" /><input type="submit" value="'.__('Update').'" /> <span id="remaining">140</span> <a href="'.BASE_URL.'upload">'.__('Upload Picture').'</a></div></form>';
